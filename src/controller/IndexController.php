...
class IndexController{
	...
	function dir(){
		...
		$path = join("/", array_map("rawurlencode", explode("/", $this->url_path)));
		$readme_file = (urldecode($path) == "/") ? "/_index" : rtrim(urldecode($path), "/");
		$readme_path = dirname(dirname(__file__))."/html_readme".$readme_file.".html";
		$readme = file_exists($readme_path) ? file_get_contents($readme_path) : NULL;
		if($_SERVER["HTTP_ACCEPT"] == "application/files+json") {
			header("Content-Type: application/files+json");
			$files = [];
			foreach($this->items as $i => $_) {
				unset($this->items[$i]["downloadUrl"]);
				$files []= $this->items[$i];
			}
			$response = array("code" => 0, "message" => "success", "data" => array("readme" => $readme, "files" => $files));
	...
	function is404(){
		...
		if($_SERVER["HTTP_ACCEPT"] == "application/files+json") {
			$json = array("code" => 404, "message" => "not found");
