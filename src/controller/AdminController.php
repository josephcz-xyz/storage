...
class AdminController{
	...
	function login(){
		...
		if (config('password') == 'key') {
			// TODO: Secure Key Auth
		}
		...
		return view::load('login')->with('title', '登录');	
	}
	...
	function settings(){
		...
		if($_POST){
			...
			config('allow_list', $_POST['allow_list']);
	...
	function install_3(){
		...
		$password = sha1($data['refresh_token'].time().rand(0, 0x7F66CCFF));
