<?php
	class onedrive{
		...
		static function dir($path="/"){
			...
			$items = array();
			$allow_list = explode(PHP_EOL,config('allow_list'));
			$allow_list = is_array($allow_list) ? $allow_list : [];
			$allow_tree = array();
			foreach($allow_list as $allow_dir){
				$allow_dir  = trim($allow_dir);
				$allow_dir = ($allow_dir == "") ? "/" : $allow_dir;
				$allow_level = ($allow_dir == "/") ? [] : explode("/", substr($allow_dir, 1));
				$current = &$allow_tree;
				for ($i = 0; $i < count($allow_level); $i++) {
					if (!isset($current[$allow_level[$i]])) $current[$allow_level[$i]] = array();
					$current = &$current[$allow_level[$i]];

				}
			}
			$allow_tree = array("" => $allow_tree);
			$path_level = ($path == "/") ? [""] : explode("/", rtrim($path, "/"));
			$current = &$allow_tree;
			for ($i = 0; $i < count($path_level); $i++) {
				$current = &$current[$path_level[$i]];
			}
			if ($current == []) {
				$is_403 = true;
				foreach($allow_list as $allow_dir) {
					$allow_dir  = trim($allow_dir);
					$path_prefix = substr($path, 0, strlen($allow_dir));
					if ($path_prefix === $allow_dir) {
						$is_403 = false;
						self::dir_next_page($request, $items);
					}
				}

				if (!$is_403) {	return $items; } else { return false; }
			}

			foreach ($current as $key => $value) {
				$parent = array();
				self::dir_next_page($request, $parent);
				if (!is_null($parent[$key])) $items[$key] = $parent[$key];
			}
			return $items;

			//以下内容是原始 dir() 函数，只能做黑名单，不能做白名单，所以不用了。
			//不在列表显示的文件夹
